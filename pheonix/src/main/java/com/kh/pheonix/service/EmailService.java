...
public class EmailService {
	...
	//인증번호 발송 - 주어진 이메일에 무작위 6자리 숫자를 전송
	public void nonUserSendCert(String nonUserCertEmail) {
		Random r = new Random();
		int number = r.nextInt(1000000); //000000~999999
		DecimalFormat fmt = new DecimalFormat("000000");
		...
		//메일 발송
		SimpleMailMessage message = new SimpleMailMessage();
		message.setTo(nonUserCertEmail);
		message.setSubject("[Pheonix] 인증번호 안내");
		message.setText("인증번호는 [" + fmt.format(number) + "] 입니다");
		...
		//인증번호 저장 - 기존 내역 삭제 후 저장
		nonUserCertDao.delete(nonUserCertEmail); // 기존의 모든 만료된 인증번호를 삭제합니다.
		NonUserCertDto nonUserCertDto = new NonUserCertDto();
		nonUserCertDto.setNonUserCertEmail(nonUserCertEmail);
		nonUserCertDto.setNonUserCertCode(fmt.format(number)); // 새로운 인증번호를 설정합니다.
		nonUserCertDao.insert(nonUserCertDto); // 새로운 인증번호를 저장합니다.
